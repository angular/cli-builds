    ...
    try {
        ...
        if (isGlobalGreater &&
            ...
            if (rawCommandName === 'update' &&
                ...
                try {
                    const { isWarningEnabled } = await Promise.resolve().then(() => __importStar(require('../src/utilities/config')));
                    if (await isWarningEnabled('versionMismatch')) {
                        ...
                        const { colors } = await Promise.resolve().then(() => __importStar(require('../src/utilities/color')));
                        // eslint-disable-next-line  no-console
